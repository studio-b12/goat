...
func (t *Executor) ExecuteGoatfile(gf goatfile.Goatfile, initialParams engine.State) (err error) {
	...
	defer func() {
		...
		for _, act := range gf.Teardown {
			err := t.executeAction(log, eng, act, gf.Path)
			if err != nil {
...
func (t *Executor) executeTest(
	...
	defer func() {
		...
		for _, postReq := range gf.TeardownEach {
			err := t.executeAction(log, eng, postReq, gf.Path)
			if err != nil {
				if act.Type() == goatfile.ActionRequest {
	...
	if t.isSkip("setup-each") {
		...
		for _, preAct := range gf.SetupEach {
			err := t.executeAction(log, eng, preAct, gf.Path)
			if err != nil {
...
func (t *Executor) executeAction(log rogu.Logger, eng engine.Engine, act goatfile.Action, gfLoc string) (err error) {
	switch act.Type() {
	case goatfile.ActionRequest:
		req := act.(goatfile.Request)
		err = t.executeRequest(eng, req)
		if err != nil {
			err = errs.WithSuffix(err, fmt.Sprintf("(%s:%d)", gfLoc, req.PosLine))
		}
		return err
